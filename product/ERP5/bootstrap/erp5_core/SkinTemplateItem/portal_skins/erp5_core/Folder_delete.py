...
if selection_name:
  ...
  if portal.portal_selections.selectionHasChanged(md5_object_uid_list, uids):
    return context.Base_renderMessage(translate("Sorry, your selection has changed."), "warning")

if not uids:
  return context.Base_renderMessage(translate("Please select one or more items first."), "warning")
...
if True:  # useless indentation
  ...
  if True:  # useless indentation adding cyclomatic complexity
    ...
    try:
      ...
      message = Base_translateString("Deleted ${count} documents", mapping={"count": len(object_list)})
